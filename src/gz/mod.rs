...
//!
//! The [`open`](mod@open) (`gzlib.c`) and [`close`](mod@close) (`gzclose.c`,
//! AAP §0.4.1) submodules have both landed on top of this `state` foundation;
//! `close` provides the top-level [`gzclose`] dispatcher. Until the remaining
//! consumer (the `extern "C"` FFI shim) lands, parts of the crate-internal
//! `GzState` API still have no in-crate caller, so

//! Static fixed Huffman decode tables ([`LENFIX`], [`DISTFIX`]) for DEFLATE
//! fixed blocks — a verbatim port of the C `inffixed.h`.
//!
//! DEFLATE (RFC 1951 §3.2.6) defines a *fixed* Huffman block type (`BTYPE = 01`)
//! whose literal/length and distance codes are constant and specified by the
//! format itself, rather than being transmitted in the stream. This module
//! provides the two pre-generated decode tables used to decode such blocks:
//!
//! * [`LENFIX`] — the fixed literal/length decode table (512 entries, indexed
//!   by the low 9 bits of the bit buffer).
//! * [`DISTFIX`] — the fixed distance decode table (32 entries, indexed by the
//!   low 5 bits of the bit buffer).
//!
//! Both tables are laid out exactly as produced by the reference zlib
//! `makefixed()` generator (which emitted the original `inffixed.h`). They are
//! transcribed here entry-for-entry so that decoding a fixed block yields
//! byte-identical output to the C library. The decode loops in `mod.rs` /
//! `back.rs` wire these tables into the inflate state via
//! `use crate::inflate::fixed::{LENFIX, DISTFIX};` (setting the C
//! `lencode`/`lenbits = 9` and `distcode`/`distbits = 5`).
//!
//! # Safety
//!
//! This module contains **zero `unsafe`** and does **not** use `std`. It is
//! pure compile-time constant data over [`Code`] (from
//! [`crate::inflate::tables`]) and is fully `no_std`-compatible.

use crate::inflate::tables::Code;

/// Terse, `const`-evaluable constructor for a [`Code`] table entry.
///
/// This local helper exists solely to keep the large [`LENFIX`]/[`DISTFIX`]
/// initializers compact and reviewable (mirroring the `{op, bits, val}`
/// brace-initializers in the C `inffixed.h`). It performs no logic and does not
/// alter [`Code`]'s layout; it is a plain field-wise wrapper. Because [`Code`]
/// is `Copy` with public fields, this is valid in `static`/`const` initializer
/// position.
#[inline]
const fn code(op: u8, bits: u8, val: u16) -> Code {
    Code { op, bits, val }
}

/// Fixed literal/length Huffman decode table for DEFLATE fixed blocks
/// (RFC 1951 §3.2.6).
///
/// Contains exactly **512** [`Code`] entries and is indexed by the low **9**
/// bits of the bit buffer (root index bits = 9). Ported verbatim from the C
/// `inffixed.h` (`lenfix[512]`), which was produced by the reference
/// `makefixed()` generator.
///
/// Note that the four entries at the indices where `(i & 127) == 99`
/// (99, 227, 355, 483) carry `op = 64` — the invalid-code marker for the fixed
/// code's two unused symbols (286/287). This matches the `makefixed()` output
/// exactly; see the generator cross-check in this module's tests for how the
/// raw [`inflate_table`](crate::inflate::tables::inflate_table) output relates
/// to these patched sentinels.
#[rustfmt::skip]
pub static LENFIX: [Code; 512] = [
    code(96, 7, 0), code(0, 8, 80), code(0, 8, 16), code(20, 8, 115), code(18, 7, 31), code(0, 8, 112), code(0, 8, 48), code(0, 9, 192),
    code(16, 7, 10), code(0, 8, 96), code(0, 8, 32), code(0, 9, 160), code(0, 8, 0), code(0, 8, 128), code(0, 8, 64), code(0, 9, 224),
    code(16, 7, 6), code(0, 8, 88), code(0, 8, 24), code(0, 9, 144), code(19, 7, 59), code(0, 8, 120), code(0, 8, 56), code(0, 9, 208),
    code(17, 7, 17), code(0, 8, 104), code(0, 8, 40), code(0, 9, 176), code(0, 8, 8), code(0, 8, 136), code(0, 8, 72), code(0, 9, 240),
    code(16, 7, 4), code(0, 8, 84), code(0, 8, 20), code(21, 8, 227), code(19, 7, 43), code(0, 8, 116), code(0, 8, 52), code(0, 9, 200),
    code(17, 7, 13), code(0, 8, 100), code(0, 8, 36), code(0, 9, 168), code(0, 8, 4), code(0, 8, 132), code(0, 8, 68), code(0, 9, 232),
    code(16, 7, 8), code(0, 8, 92), code(0, 8, 28), code(0, 9, 152), code(20, 7, 83), code(0, 8, 124), code(0, 8, 60), code(0, 9, 216),
    code(18, 7, 23), code(0, 8, 108), code(0, 8, 44), code(0, 9, 184), code(0, 8, 12), code(0, 8, 140), code(0, 8, 76), code(0, 9, 248),
    code(16, 7, 3), code(0, 8, 82), code(0, 8, 18), code(21, 8, 163), code(19, 7, 35), code(0, 8, 114), code(0, 8, 50), code(0, 9, 196),
    code(17, 7, 11), code(0, 8, 98), code(0, 8, 34), code(0, 9, 164), code(0, 8, 2), code(0, 8, 130), code(0, 8, 66), code(0, 9, 228),
    code(16, 7, 7), code(0, 8, 90), code(0, 8, 26), code(0, 9, 148), code(20, 7, 67), code(0, 8, 122), code(0, 8, 58), code(0, 9, 212),
    code(18, 7, 19), code(0, 8, 106), code(0, 8, 42), code(0, 9, 180), code(0, 8, 10), code(0, 8, 138), code(0, 8, 74), code(0, 9, 244),
    code(16, 7, 5), code(0, 8, 86), code(0, 8, 22), code(64, 8, 0), code(19, 7, 51), code(0, 8, 118), code(0, 8, 54), code(0, 9, 204),
    code(17, 7, 15), code(0, 8, 102), code(0, 8, 38), code(0, 9, 172), code(0, 8, 6), code(0, 8, 134), code(0, 8, 70), code(0, 9, 236),
    code(16, 7, 9), code(0, 8, 94), code(0, 8, 30), code(0, 9, 156), code(20, 7, 99), code(0, 8, 126), code(0, 8, 62), code(0, 9, 220),
    code(18, 7, 27), code(0, 8, 110), code(0, 8, 46), code(0, 9, 188), code(0, 8, 14), code(0, 8, 142), code(0, 8, 78), code(0, 9, 252),
    code(96, 7, 0), code(0, 8, 81), code(0, 8, 17), code(21, 8, 131), code(18, 7, 31), code(0, 8, 113), code(0, 8, 49), code(0, 9, 194),
    code(16, 7, 10), code(0, 8, 97), code(0, 8, 33), code(0, 9, 162), code(0, 8, 1), code(0, 8, 129), code(0, 8, 65), code(0, 9, 226),
    code(16, 7, 6), code(0, 8, 89), code(0, 8, 25), code(0, 9, 146), code(19, 7, 59), code(0, 8, 121), code(0, 8, 57), code(0, 9, 210),
    code(17, 7, 17), code(0, 8, 105), code(0, 8, 41), code(0, 9, 178), code(0, 8, 9), code(0, 8, 137), code(0, 8, 73), code(0, 9, 242),
    code(16, 7, 4), code(0, 8, 85), code(0, 8, 21), code(16, 8, 258), code(19, 7, 43), code(0, 8, 117), code(0, 8, 53), code(0, 9, 202),
    code(17, 7, 13), code(0, 8, 101), code(0, 8, 37), code(0, 9, 170), code(0, 8, 5), code(0, 8, 133), code(0, 8, 69), code(0, 9, 234),
    code(16, 7, 8), code(0, 8, 93), code(0, 8, 29), code(0, 9, 154), code(20, 7, 83), code(0, 8, 125), code(0, 8, 61), code(0, 9, 218),
    code(18, 7, 23), code(0, 8, 109), code(0, 8, 45), code(0, 9, 186), code(0, 8, 13), code(0, 8, 141), code(0, 8, 77), code(0, 9, 250),
    code(16, 7, 3), code(0, 8, 83), code(0, 8, 19), code(21, 8, 195), code(19, 7, 35), code(0, 8, 115), code(0, 8, 51), code(0, 9, 198),
    code(17, 7, 11), code(0, 8, 99), code(0, 8, 35), code(0, 9, 166), code(0, 8, 3), code(0, 8, 131), code(0, 8, 67), code(0, 9, 230),
    code(16, 7, 7), code(0, 8, 91), code(0, 8, 27), code(0, 9, 150), code(20, 7, 67), code(0, 8, 123), code(0, 8, 59), code(0, 9, 214),
    code(18, 7, 19), code(0, 8, 107), code(0, 8, 43), code(0, 9, 182), code(0, 8, 11), code(0, 8, 139), code(0, 8, 75), code(0, 9, 246),
    code(16, 7, 5), code(0, 8, 87), code(0, 8, 23), code(64, 8, 0), code(19, 7, 51), code(0, 8, 119), code(0, 8, 55), code(0, 9, 206),
    code(17, 7, 15), code(0, 8, 103), code(0, 8, 39), code(0, 9, 174), code(0, 8, 7), code(0, 8, 135), code(0, 8, 71), code(0, 9, 238),
    code(16, 7, 9), code(0, 8, 95), code(0, 8, 31), code(0, 9, 158), code(20, 7, 99), code(0, 8, 127), code(0, 8, 63), code(0, 9, 222),
    code(18, 7, 27), code(0, 8, 111), code(0, 8, 47), code(0, 9, 190), code(0, 8, 15), code(0, 8, 143), code(0, 8, 79), code(0, 9, 254),
    code(96, 7, 0), code(0, 8, 80), code(0, 8, 16), code(20, 8, 115), code(18, 7, 31), code(0, 8, 112), code(0, 8, 48), code(0, 9, 193),
    code(16, 7, 10), code(0, 8, 96), code(0, 8, 32), code(0, 9, 161), code(0, 8, 0), code(0, 8, 128), code(0, 8, 64), code(0, 9, 225),
    code(16, 7, 6), code(0, 8, 88), code(0, 8, 24), code(0, 9, 145), code(19, 7, 59), code(0, 8, 120), code(0, 8, 56), code(0, 9, 209),
    code(17, 7, 17), code(0, 8, 104), code(0, 8, 40), code(0, 9, 177), code(0, 8, 8), code(0, 8, 136), code(0, 8, 72), code(0, 9, 241),
    code(16, 7, 4), code(0, 8, 84), code(0, 8, 20), code(21, 8, 227), code(19, 7, 43), code(0, 8, 116), code(0, 8, 52), code(0, 9, 201),
    code(17, 7, 13), code(0, 8, 100), code(0, 8, 36), code(0, 9, 169), code(0, 8, 4), code(0, 8, 132), code(0, 8, 68), code(0, 9, 233),
    code(16, 7, 8), code(0, 8, 92), code(0, 8, 28), code(0, 9, 153), code(20, 7, 83), code(0, 8, 124), code(0, 8, 60), code(0, 9, 217),
    code(18, 7, 23), code(0, 8, 108), code(0, 8, 44), code(0, 9, 185), code(0, 8, 12), code(0, 8, 140), code(0, 8, 76), code(0, 9, 249),
    code(16, 7, 3), code(0, 8, 82), code(0, 8, 18), code(21, 8, 163), code(19, 7, 35), code(0, 8, 114), code(0, 8, 50), code(0, 9, 197),
    code(17, 7, 11), code(0, 8, 98), code(0, 8, 34), code(0, 9, 165), code(0, 8, 2), code(0, 8, 130), code(0, 8, 66), code(0, 9, 229),
    code(16, 7, 7), code(0, 8, 90), code(0, 8, 26), code(0, 9, 149), code(20, 7, 67), code(0, 8, 122), code(0, 8, 58), code(0, 9, 213),
    code(18, 7, 19), code(0, 8, 106), code(0, 8, 42), code(0, 9, 181), code(0, 8, 10), code(0, 8, 138), code(0, 8, 74), code(0, 9, 245),
    code(16, 7, 5), code(0, 8, 86), code(0, 8, 22), code(64, 8, 0), code(19, 7, 51), code(0, 8, 118), code(0, 8, 54), code(0, 9, 205),
    code(17, 7, 15), code(0, 8, 102), code(0, 8, 38), code(0, 9, 173), code(0, 8, 6), code(0, 8, 134), code(0, 8, 70), code(0, 9, 237),
    code(16, 7, 9), code(0, 8, 94), code(0, 8, 30), code(0, 9, 157), code(20, 7, 99), code(0, 8, 126), code(0, 8, 62), code(0, 9, 221),
    code(18, 7, 27), code(0, 8, 110), code(0, 8, 46), code(0, 9, 189), code(0, 8, 14), code(0, 8, 142), code(0, 8, 78), code(0, 9, 253),
    code(96, 7, 0), code(0, 8, 81), code(0, 8, 17), code(21, 8, 131), code(18, 7, 31), code(0, 8, 113), code(0, 8, 49), code(0, 9, 195),
    code(16, 7, 10), code(0, 8, 97), code(0, 8, 33), code(0, 9, 163), code(0, 8, 1), code(0, 8, 129), code(0, 8, 65), code(0, 9, 227),
    code(16, 7, 6), code(0, 8, 89), code(0, 8, 25), code(0, 9, 147), code(19, 7, 59), code(0, 8, 121), code(0, 8, 57), code(0, 9, 211),
    code(17, 7, 17), code(0, 8, 105), code(0, 8, 41), code(0, 9, 179), code(0, 8, 9), code(0, 8, 137), code(0, 8, 73), code(0, 9, 243),
    code(16, 7, 4), code(0, 8, 85), code(0, 8, 21), code(16, 8, 258), code(19, 7, 43), code(0, 8, 117), code(0, 8, 53), code(0, 9, 203),
    code(17, 7, 13), code(0, 8, 101), code(0, 8, 37), code(0, 9, 171), code(0, 8, 5), code(0, 8, 133), code(0, 8, 69), code(0, 9, 235),
    code(16, 7, 8), code(0, 8, 93), code(0, 8, 29), code(0, 9, 155), code(20, 7, 83), code(0, 8, 125), code(0, 8, 61), code(0, 9, 219),
    code(18, 7, 23), code(0, 8, 109), code(0, 8, 45), code(0, 9, 187), code(0, 8, 13), code(0, 8, 141), code(0, 8, 77), code(0, 9, 251),
    code(16, 7, 3), code(0, 8, 83), code(0, 8, 19), code(21, 8, 195), code(19, 7, 35), code(0, 8, 115), code(0, 8, 51), code(0, 9, 199),
    code(17, 7, 11), code(0, 8, 99), code(0, 8, 35), code(0, 9, 167), code(0, 8, 3), code(0, 8, 131), code(0, 8, 67), code(0, 9, 231),
    code(16, 7, 7), code(0, 8, 91), code(0, 8, 27), code(0, 9, 151), code(20, 7, 67), code(0, 8, 123), code(0, 8, 59), code(0, 9, 215),
    code(18, 7, 19), code(0, 8, 107), code(0, 8, 43), code(0, 9, 183), code(0, 8, 11), code(0, 8, 139), code(0, 8, 75), code(0, 9, 247),
    code(16, 7, 5), code(0, 8, 87), code(0, 8, 23), code(64, 8, 0), code(19, 7, 51), code(0, 8, 119), code(0, 8, 55), code(0, 9, 207),
    code(17, 7, 15), code(0, 8, 103), code(0, 8, 39), code(0, 9, 175), code(0, 8, 7), code(0, 8, 135), code(0, 8, 71), code(0, 9, 239),
    code(16, 7, 9), code(0, 8, 95), code(0, 8, 31), code(0, 9, 159), code(20, 7, 99), code(0, 8, 127), code(0, 8, 63), code(0, 9, 223),
    code(18, 7, 27), code(0, 8, 111), code(0, 8, 47), code(0, 9, 191), code(0, 8, 15), code(0, 8, 143), code(0, 8, 79), code(0, 9, 255),
];

/// Fixed distance Huffman decode table for DEFLATE fixed blocks
/// (RFC 1951 §3.2.6).
///
/// Contains exactly **32** [`Code`] entries and is indexed by the low **5**
/// bits of the bit buffer (root index bits = 5). Ported verbatim from the C
/// `inffixed.h` (`distfix[32]`). The two entries at indices 15 and 31 carry
/// `op = 64` (invalid-code markers for the two unused distance symbols 30/31).
#[rustfmt::skip]
pub static DISTFIX: [Code; 32] = [
    code(16, 5, 1), code(23, 5, 257), code(19, 5, 17), code(27, 5, 4097),
    code(17, 5, 5), code(25, 5, 1025), code(21, 5, 65), code(29, 5, 16385),
    code(16, 5, 3), code(24, 5, 513), code(20, 5, 33), code(28, 5, 8193),
    code(18, 5, 9), code(26, 5, 2049), code(22, 5, 129), code(64, 5, 0),
    code(16, 5, 2), code(23, 5, 385), code(19, 5, 25), code(27, 5, 6145),
    code(17, 5, 7), code(25, 5, 1537), code(21, 5, 97), code(29, 5, 24577),
    code(16, 5, 4), code(24, 5, 769), code(20, 5, 49), code(28, 5, 12289),
    code(18, 5, 13), code(26, 5, 3073), code(22, 5, 193), code(64, 5, 0),
];

#[cfg(test)]
mod tests {
    use super::*;
    use crate::inflate::tables::{CodeType, ENOUGH, ENOUGH_DISTS, ENOUGH_LENS, inflate_table};

    /// Both tables must have the exact lengths mandated by the fixed-code root
    /// index widths (2^9 and 2^5).
    #[test]
    fn table_lengths_are_exact() {
        assert_eq!(LENFIX.len(), 512);
        assert_eq!(DISTFIX.len(), 32);
    }

    /// Anchor entries confirmed against the C `inffixed.h` source.
    #[test]
    fn anchor_entries_match_source() {
        assert_eq!(
            LENFIX[0],
            Code {
                op: 96,
                bits: 7,
                val: 0
            }
        );
        assert_eq!(
            LENFIX[511],
            Code {
                op: 0,
                bits: 9,
                val: 255
            }
        );
        assert_eq!(
            DISTFIX[0],
            Code {
                op: 16,
                bits: 5,
                val: 1
            }
        );
        assert_eq!(
            DISTFIX[15],
            Code {
                op: 64,
                bits: 5,
                val: 0
            }
        );
        assert_eq!(
            DISTFIX[31],
            Code {
                op: 64,
                bits: 5,
                val: 0
            }
        );
    }

    /// The four `makefixed()` invalid-code sentinels in `LENFIX` sit at the
    /// indices where `(i & 127) == 99` and carry `op = 64`, `bits = 8`,
    /// `val = 0`.
    #[test]
    fn lenfix_makefixed_sentinels_are_op64() {
        for i in [99usize, 227, 355, 483] {
            assert_eq!(
                LENFIX[i],
                Code {
                    op: 64,
                    bits: 8,
                    val: 0
                },
                "expected makefixed op=64 sentinel at LENFIX[{i}]"
            );
        }
    }

    /// The complete 32-entry distance table, transcribed from `inffixed.h`
    /// `distfix[32]`. `DISTFIX` has no `makefixed()` patch, so it is checked
    /// entry-for-entry.
    #[rustfmt::skip]
    #[test]
    fn distfix_full_table_matches_source() {
        let expected: [Code; 32] = [
            code(16, 5, 1), code(23, 5, 257), code(19, 5, 17), code(27, 5, 4097),
            code(17, 5, 5), code(25, 5, 1025), code(21, 5, 65), code(29, 5, 16385),
            code(16, 5, 3), code(24, 5, 513), code(20, 5, 33), code(28, 5, 8193),
            code(18, 5, 9), code(26, 5, 2049), code(22, 5, 129), code(64, 5, 0),
            code(16, 5, 2), code(23, 5, 385), code(19, 5, 25), code(27, 5, 6145),
            code(17, 5, 7), code(25, 5, 1537), code(21, 5, 97), code(29, 5, 24577),
            code(16, 5, 4), code(24, 5, 769), code(20, 5, 49), code(28, 5, 12289),
            code(18, 5, 13), code(26, 5, 3073), code(22, 5, 193), code(64, 5, 0),
        ];
        assert_eq!(DISTFIX, expected);
    }

    /// Build the fixed literal/length code lengths per RFC 1951 §3.2.6:
    /// symbols 0..144 -> 8 bits, 144..256 -> 9, 256..280 -> 7, 280..288 -> 8.
    fn fixed_litlen_lengths() -> [u16; 288] {
        let mut lens = [0u16; 288];
        lens[0..144].fill(8);
        lens[144..256].fill(9);
        lens[256..280].fill(7);
        lens[280..288].fill(8);
        lens
    }

    /// **Generator cross-check (the single most valuable safeguard).**
    ///
    /// Rebuild the fixed literal/length table from first principles using
    /// [`inflate_table`] and assert it reproduces [`LENFIX`]. The only
    /// permitted difference is at the four `(i & 127) == 99` indices: the C
    /// `makefixed()` generator overwrote those with `op = 64`, whereas the raw
    /// builder emits the length/extra codes for the fixed code's two unused
    /// symbols (286 -> `lext[29] = 68`, 287 -> `lext[30] = 193`). Both of those
    /// still have the invalid-code bit (64) set, so decoding behaves
    /// identically; only the exact `op` byte differs. Everything else must
    /// match bit-for-bit.
    #[test]
    fn lenfix_matches_inflate_table_generator() {
        let lens = fixed_litlen_lengths();
        let mut table = [Code::default(); ENOUGH];
        let mut work = [0u16; 288];
        let mut index = 0usize;
        let mut bits = 9usize;

        let res = inflate_table(
            CodeType::Lens,
            &lens,
            288,
            &mut table,
            &mut index,
            &mut bits,
            &mut work,
        );
        assert_eq!(res, Ok(()));
        assert_eq!(bits, 9, "fixed lit/len root index bits must stay 9");
        assert_eq!(index, 512, "fixed lit/len table must occupy 1 << 9 entries");
        assert!(index <= ENOUGH_LENS, "must fit within ENOUGH_LENS");

        for i in 0..512 {
            // `bits` and `val` must always match the generated table.
            assert_eq!(table[i].bits, LENFIX[i].bits, "bits mismatch at index {i}");
            assert_eq!(table[i].val, LENFIX[i].val, "val mismatch at index {i}");
            if (i & 127) == 99 {
                // makefixed() sentinel: LENFIX carries op=64, the raw builder
                // carries 68 or 193 (both with the 64 invalid-code bit set).
                assert_eq!(LENFIX[i].op, 64, "LENFIX sentinel op must be 64 at {i}");
                assert!(
                    table[i].op == 68 || table[i].op == 193,
                    "expected raw invalid-marker op (68 or 193) at {i}, got {}",
                    table[i].op
                );
                assert_ne!(table[i].op & 64, 0, "invalid-code bit must be set at {i}");
            } else {
                assert_eq!(table[i].op, LENFIX[i].op, "op mismatch at index {i}");
            }
        }
    }

    /// **Generator cross-check for the distance table.**
    ///
    /// The fixed distance code is 32 symbols each of length 5. Rebuilding it
    /// via [`inflate_table`] must reproduce [`DISTFIX`] exactly — there is no
    /// `makefixed()` patch for the distance table.
    #[test]
    fn distfix_matches_inflate_table_generator() {
        let lens = [5u16; 32];
        let mut table = [Code::default(); ENOUGH];
        let mut work = [0u16; 32];
        let mut index = 0usize;
        let mut bits = 5usize;

        let res = inflate_table(
            CodeType::Dists,
            &lens,
            32,
            &mut table,
            &mut index,
            &mut bits,
            &mut work,
        );
        assert_eq!(res, Ok(()));
        assert_eq!(bits, 5, "fixed distance root index bits must stay 5");
        assert_eq!(index, 32, "fixed distance table must occupy 1 << 5 entries");
        assert!(index <= ENOUGH_DISTS, "must fit within ENOUGH_DISTS");

        for i in 0..32 {
            assert_eq!(table[i], DISTFIX[i], "distfix mismatch at index {i}");
        }
    }
}
